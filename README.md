...
This repository is the **single source of truth** for each specification. Each spec is versioned here as Markdown; open questions and feedback are discussed in the **[Discussions](../../discussions)** tab, with one hub thread per specification acting as a signpost back to the canonical document.
...
| 💬 **Discussion** | [Discussions](../../discussions) tab | One pinned hub thread per spec. Asynchronous feedback, open questions, decisions. The thread links back to the spec; the spec links to the thread. |
...
- **Raise an open question or give feedback** — comment in the spec's hub thread in [Discussions](../../discussions).
- **Propose a new spec** — open a discussion describing the topic; once there is rough consensus, it is added as `specs/<nn-slug>/`.

## Status values

- **Draft** — under active development; structure and content may change.
- **Review** — content considered complete; gathering final feedback.
- **Stable** — agreed; changes are tracked as new versions.
